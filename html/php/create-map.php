...
if (empty($_SESSION['logged_in'])) {
  header("Location: ../create_map.php?msg=nologin");
  exit();
}

// Don't allow direct access
if (!isset($_POST['create-map-button'])) {
  header("Location: ../create_map.php?msg=noauth");
  exit();
}

// Don't accept empty map type
if (empty($_POST['create-map-name'])) {
  header("Location: ../create_map.php?msg=create_map-noname");
  exit();
}

// !Match name of alphanumerical characters
if (!preg_match("/^[a-zA-Z0-9_öäå]+$/", $_POST['create-map-name'])) {
  header("Location: ../create_map.php?msg=create_map-ill_nick");
  exit();
}
  ...
  if (isset($_POST[$municipality . "-checkbox"])) { // If checked
    if (!empty($_POST[$municipality . "-count"])) { // If count is set
      if (!preg_match("/^[0-9]+$/", $_POST[$municipality . "-count"])) {
        header("Location: ../create_map.php?msg=create_map-nan_input");
        exit();
      } else {
...
if (mapExists($nickname, $previous_map_name)) {
  if (!updateExistingMap($nickname, $previous_map_name, $filetype, $isPrivate)) {
    header("Location: ../create_map.php?msg=update_map-failed");
    exit();
  }
} else if (!insertNewMap($nickname, $filetype, $isPrivate)) {
  header("Location: ../create_map.php?msg=insert_map-failed");
  exit();
}
...
if (empty($handle)) {
  header("Location: ../create_map.php?msg=txt_failed");
  exit();
}
...
header("Location: ../maps.php?msg=create_map-success");
exit();