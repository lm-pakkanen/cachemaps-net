...
if (!empty($_SESSION['finds_list'])) {
  $finds_list = $_SESSION['finds_list'];
  unset($_SESSION['finds_list']);
}

$isPrivate = $_SESSION['mapIsPrivate'];
unset($_SESSION['mapIsPrivate']);

$_SESSION['previous_map_name'] = explode(".", $finds_list[0])[0];
?>
...
<head>
  <!-- Title & meta -->
  <title>CacheMaps - Update a map</title>
  <meta name="description" content="Update a map">
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <!-- SEO -->
  <link rel="canonical" href="https://www.CacheMaps.net/update_map.php">
  <!-- Icons -->
    ...
    <div id="page-shader">
      ...
      <div class="main">
        <div class="container-custom">
          <form action="php/update-map.php" method="POST">
            <div class="title">
              Create a new map!
            </div>
            <hr class="line">
            <div class="map-options">
              <div class="isPrivate-wrapper">
                <label class="isPrivate-label" for="isPrivate">Private map</label>
                <input type="checkbox" id="isPrivate" name="isPrivate" <?PHP
                if (!empty($isPrivate)) {
                  echo "checked";
                }
                ?> />
              ...
              <div>
                <input class="map-name" name="create-map-name" type="text" placeholder="enter a map name..." <?PHP
                if (!empty($finds_list)) {
                  echo 'value="' . explode(".", $finds_list[0])[0] . '"';
                }
                ?> />
                <input class="button-default" name="create-map-button" type="submit" value="Create map" />
              </div>
            </div>
            <div class="map-options" id="search-wrapper">
              <input class="municipality-search" id="search" type="search" placeholder="find by name..." />
            </div>
            <div class="municipalities-holder">
              <?PHP
              foreach ($municipalities as $municipality) {
                foreach ($finds_list as $find) {
                  $find = explode(",", $find);

                  $find_name = $find[0];
                  $find_count = $find[1];

                  if ($find_name === $municipality) {
                    $checked = "checked";
                    $count = $find_count;
                    break;
                  } else {
                ...
                echo '        <input class="municipality-checkbox" type="checkbox" id="' . $municipality . '-checkbox" name="' . $municipality . '-checkbox" ' . $checked . '>';
                echo '        <label class="municipality-label" for="' . $municipality . '-checkbox">' . $municipality . '</label>';
                echo '    </div>';
                echo '    <input class="municipality-count" type="number" placeholder="count (optional)" name="' . $municipality . '-count" value="' . $count . '">';
                echo '</div>';
              }
